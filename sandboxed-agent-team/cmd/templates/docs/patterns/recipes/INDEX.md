...
| [java-lit-bridge.md](java-lit-bridge.md) | Wrapping browser APIs as Flow components: `@Tag`/`@JsModule`-annotated Java `Component` subclass + matching `LitElement`; `@DomEvent` typed events; `@EventData` client-side expression extraction; light DOM vs shadow DOM for Lumo theme continuity; `bubbles`/`composed` event dispatch. |
| [testbench-browserless.md](testbench-browserless.md) | Parallel browserless UI tests with `BrowserlessTest` + `ComponentTester<T>`: Maven dependency, parallel surefire configuration, three-section page object structure, slot-child access, keyboard shortcut testing. |
| [testbench-e2e-server.md](testbench-e2e-server.md) | Maven `it` profile (Jetty start/stop, failsafe, IT port 9090) and `ServerExtension` (embedded Jetty for IDE runs, reference-counted, `waitUntilReady()`, Maven-vs-IDE auto-detect via port check). Foundation recipe for TestBench E2E tests. |
| [testbench-e2e-parallel.md](testbench-e2e-parallel.md) | Parallel TestBench E2E execution: `TestBenchParallelLimiter` `LauncherSessionListener` SPI, `junit-platform.properties` with `ParallelConfigurationStrategy`, concurrent limit wired through POM property → failsafe `systemPropertyVariables` → `it-test.properties`. Layered on `testbench-e2e-server`. |
